//
//  Bildschneider_RectangleCropView.m
//  Bildschneider
//
...
@interface Bildschneider_RectangleCropView() {
    CGPoint lastPoint;
...
@property (nonatomic, strong) UIView *activePointlastIndexPoint;
@property (nonatomic, strong) UIView *activePointnextIndexPoint;

+ (CGPoint)convertCGPoint:(CGPoint)point1 fromRect1:(CGSize)rect1 toRect2:(CGSize)rect2;

@end

@implementation Bildschneider_RectangleCropView
...
- (UIImage *)deleteBackgroundOfImage:(UIImageView *)image
{
    ...
    
    {
        ...
        CGPoint p1 = [Bildschneider_RectangleCropView convertCGPoint:[[points objectAtIndex:0] CGPointValue] fromRect1:image.frame.size toRect2:image.image.size];
        [aPath moveToPoint:CGPointMake(p1.x, p1.y)];
        
        for (uint i=1; i<points.count; i++)
        {
            CGPoint p = [Bildschneider_RectangleCropView convertCGPoint:[[points objectAtIndex:i] CGPointValue] fromRect1:image.frame.size toRect2:image.image.size];
...
- (void)touchesBegan:(NSSet *)touches withEvent:(UIEvent *)event
{
    ...
    for (UIView *point in self.points)
    {
        ...
        if ([point pointInside:viewPoint withEvent:event])
        {
            ...
            activePointIndex = [self.points indexOfObject:point];
            self.activePointlastIndexPoint = [self.points objectAtIndex:(activePointIndex + 4 - 1) % 4];
            self.activePointnextIndexPoint = [self.points objectAtIndex:(activePointIndex + 1) % 4];
            break;
        }
    }
    
    lastPoint = locationPoint;
    //NSLog(@"touchBeganEnded: %f, %f", lastPoint.x, lastPoint.y);
}
...
- (void)touchesMoved:(NSSet *)touches withEvent:(UIEvent *)event
{
    ...
    else
    {
        ...
        switch (activePointIndex) {
            case 0:
                self.activePointlastIndexPoint.frame = CGRectMake(locationPoint.x -k_POINT_WIDTH/2, self.activePointlastIndexPoint.frame.origin.y, k_POINT_WIDTH, k_POINT_WIDTH);
                self.activePointnextIndexPoint.frame = CGRectMake(self.activePointnextIndexPoint.frame.origin.x, locationPoint.y -k_POINT_WIDTH/2, k_POINT_WIDTH, k_POINT_WIDTH);
                break;
            case 1:
                self.activePointlastIndexPoint.frame = CGRectMake(self.activePointlastIndexPoint.frame.origin.x, locationPoint.y -k_POINT_WIDTH/2, k_POINT_WIDTH, k_POINT_WIDTH);
                self.activePointnextIndexPoint.frame = CGRectMake(locationPoint.x -k_POINT_WIDTH/2, self.activePointnextIndexPoint.frame.origin.y, k_POINT_WIDTH, k_POINT_WIDTH);
                break;
            case 2:
                self.activePointlastIndexPoint.frame = CGRectMake(locationPoint.x -k_POINT_WIDTH/2, self.activePointlastIndexPoint.frame.origin.y, k_POINT_WIDTH, k_POINT_WIDTH);
                self.activePointnextIndexPoint.frame = CGRectMake(self.activePointnextIndexPoint.frame.origin.x, locationPoint.y -k_POINT_WIDTH/2, k_POINT_WIDTH, k_POINT_WIDTH);
    ...
    lastPoint = locationPoint;
    //NSLog(@"touchMoveEnded: %f, %f", lastPoint.x, lastPoint.y);
}
